...
def macfind_g(fi,patch_threshold):
    ...
    patches_cleaned = morphology.remove_small_objects(segmentation, patch_threshold)
    ...
    centroidx=np.arange(lab_num).astype(np.float64)
    centroidy=np.arange(lab_num).astype(np.float64)
    filledarea=np.arange(lab_num).astype(np.float64)
    perimeter=np.arange(lab_num).astype(np.float64)
    diameter=np.arange(lab_num).astype(np.float64)

    for i in np.arange(lab_num):
    ...
    #calculate min/max distaces of centroids
    mindist=np.arange(lab_num).astype(np.float64)
    maxdist=np.arange(lab_num).astype(np.float64)

    for i in np.arange(lab_num):
        cxm = np.ma.array(centroidx, mask=False)
        cym = np.ma.array(centroidy, mask=False)
        cxm.mask[i] = True
        cym.mask[i] = True
        idx=np.argmin((cxm - centroidx[i])**2 + (cym - centroidy[i])**2)
        mindist[i]=np.sqrt((cxm[idx] - centroidx[i])**2 + (cym[idx] - centroidy[i])**2)
        idx=np.argmax((cxm - centroidx[i])**2 + (cym - centroidy[i])**2)
        maxdist[i]=np.sqrt((cxm[idx] - centroidx[i])**2 + (cym[idx] - centroidy[i])**2)    
    
    tot_size=np.float64(im.shape[0])*np.float64(im.shape[1])
    patch_def=pd.DataFrame([dict(no=lab_num, share=np.sum(filledarea)/tot_size, 
    ...
    plt.contour(patches_cleaned, [0.5], linewidths=1.2, colors='y')
    plt.axis('off')
...
def mac_matrix_setup(mac,mc):
    ...
    for i in np.arange(len(vertices[:-1])):
        ...
        if len(idx)<2:
            ...
            idy=np.random.randint(min(idx)-1,max(idx), size=len(idz))
            mxsoil[idz]=idx[idy]
            #note: trust parameter unused
    ...
    #wow, the domain is set. however, consider much more srictly defining the macropore contact areas (DEBUG)
